...
//Κλάση για την εμφάνιση όλων των έργων της εταιρίας μέσω γραφικής διεπαφής
public class PrintProjWindow {
    ...
    PrintProjWindow(Company c){ //είσοδος του object company διαμέσο του Constructor
        ...
        String [] colNames = {"Name of the Project", "Description"};

        //Δημιουργία δισδιάστατου πίνακα που θα χρησιμοποιηθεί για την απεικόνιση των δεδομένων της ArrayList <ProjectDetail>
        String [][] dataVal = new String [c.getProjects().size()][2];

        for(int i = 0; i < c.getProjects().size(); i++){
            //ενημέρωση γραμμής με τα στοιχεία του Project
            dataVal [i][0] = c.getProjects().get(i).getProjectName();
            dataVal[i][1] = c.getProjects().get(i).getProjectDescription();
        }
